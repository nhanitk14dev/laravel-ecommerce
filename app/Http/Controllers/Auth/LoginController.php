    ...
    try {
      ...
      if (count($productsLastView)) {
        foreach ($productsLastView as $key => $value) {
          ...
          if ($checkLastView) {
            DB::table('shop_product_recent_view')->where('user_id', $uID)->where('product_id', $key)->update(['created_at' => date('Y-m-d H:i:s', strtotime($value))]);
          } else {
            DB::table('shop_product_recent_view')->insert(['user_id' => $uID, 'product_id' => $key, 'created_at' => date('Y-m-d H:i:s', strtotime($value))]);
          }
        }

        //Nếu có lưu giỏ hàng khi đăng nhập trở về trang thanh toán.
