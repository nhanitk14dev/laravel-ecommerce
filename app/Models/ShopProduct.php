...
//End type
  ...
  public function showPrice($id = null) {
    ...
    if ($special) {
      //Show price origin
      if ((int) Config::select('value')->where('key', 'product_display_special')->first()->value == 1) {
        return '<span itemprop="price" class="price_sale">' . number_format($special->price) . ' VNĐ</span>' . " " . '<del><span class="price_origin">' . number_format($this->find($id)->price) . ' VNĐ</span></del>';
      } else {
        return '<span itemprop="price" class="price_sale">' . number_format($special->price) . ' VNĐ</span>';
      }
